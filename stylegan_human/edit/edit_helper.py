...
def encoder_ifg(G, noise, attr_name, truncation=1, truncation_latent=None, 
    ...
    style_space = [G.conv1.conv.modulation(latent[:, 0])]
    i = 1
    for conv1, conv2, noise1, noise2, to_rgb in zip(
        G.convs[::2], G.convs[1::2], noise[1::2], noise[2::2], G.to_rgbs
    ):
        style_space.extend(
            (
                conv1.conv.modulation(latent[:, i]),
                conv2.conv.modulation(latent[:, i + 1]),
            )
        )
        i += 2
    ...
    for i in range(15):
        style_vect = load_pkl(
            os.path.join(latent_dir, f'{attr_name}/style_vect_mean_{i}.pkl')
        )
...
def encoder_ss(G, noise, attr_name, truncation=1, truncation_latent=None, 
    ...
    statis_dir = os.path.join(statics_dir, f"{attr_name}_statis/{layer}")
    ...
    style_vect = load_pkl(
        os.path.join(latent_dir, f'{attr_name}/style_vect_mean_{layer}.pkl')
    )
    style_vect = torch.from_numpy(style_vect).to(latent.device).float()
...
def encoder_sefa(G, noise, attr_name, truncation=1, truncation_latent=None, 
    ...
    sefa_vect = (
        torch.load(os.path.join(latent_dir, f'{attr_name}.pt'))
        .to(latent.device)
        .float()
    )
    if step != 0 and total != 0:
